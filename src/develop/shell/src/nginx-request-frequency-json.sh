...
#
# 作用
#     telegraf监控，使用path;domain;ip分析客户端经常请求的接口 - （是异常）
...
tail -n 10000 "${nginx_log}" | # 为了性能
    grep -v -E '^127\.0\.0\.1.+' | # 排除自动执行
    grep -F ' POST ' | # 只关注POST
    grep -F "${tag}" | # 获取上一秒的完整信息
    awk -F '"' '{print $2" "$(NF-1)}' | awk '{print $3";"$1";"$NF}' | # 简单获取path;domain;ip
    sed 's/\?.*;/;/g' | # 移除http请求路径参数信息（通常像 GET /path/id?u=100 改为 /path/id）
